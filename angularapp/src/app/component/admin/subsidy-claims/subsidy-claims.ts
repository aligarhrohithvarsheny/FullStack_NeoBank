...
export class SubsidyClaims implements OnInit {
  ...
  applyFilters() {
    this.filteredClaims = this.claims.filter(claim => {
      ...
      const matchesSearch = !this.searchTerm || 
        ...
        (claim.customerId && claim.customerId.includes(this.searchTerm));
  ...
  loadUserDetails(accountNumber: string) {
    // Load user details
    this.http.get(`${environment.apiBaseUrl}/api/users/account/${accountNumber}`).subscribe({
      next: (user: any) => {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/transactions/account/${accountNumber}`).subscribe({
