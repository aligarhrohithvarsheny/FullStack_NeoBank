...
export class Kyc implements OnInit {
  ...
  approve(request: KycRequest) {
    if (confirm(`Approve KYC for ${request.userName} (${request.userAccountNumber})?`)) {
      ...
      this.http.put(`${environment.apiBaseUrl}/api/kyc/approve/${request.id}?adminName=Admin`, {}).subscribe({
        next: (response: any) => {
  ...
  downloadAadharDocument(kycId: string | undefined) {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/kyc/${kycId}/aadhar-document`, {
      responseType: 'blob'
    }).subscribe({
      next: (blob: Blob) => {
  ...
  downloadPanDocument(kycId: string | undefined) {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/kyc/${kycId}/pan-document`, {
      responseType: 'blob'
    }).subscribe({
      next: (blob: Blob) => {
        const url = window.URL.createObjectURL(blob);
  ...
  reject(request: KycRequest) {
    if (confirm(`Reject KYC for ${request.userName} (${request.userAccountNumber})?`)) {
      // Update KYC request in MySQL database
      this.http.put(`${environment.apiBaseUrl}/api/kyc/reject/${request.id}?adminName=Admin`, {}).subscribe({
        next: (response: any) => {
