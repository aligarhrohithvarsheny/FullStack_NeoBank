...
export class Loans implements OnInit {
  ...
  approve(loan: LoanRequest) {
    ...
    this.http.put(`${environment.apiBaseUrl}/api/loans/${loanId}/approve?status=Approved`, {}).subscribe({
      next: (response: any) => {
  ...
  reject(loan: LoanRequest) {
    ...
    this.http.put(`${environment.apiBaseUrl}/api/loans/${loanId}/approve?status=Rejected`, {}).subscribe({
      next: (response: any) => {
...
✅ User can now see the updated balance and transaction history!
  ...
  calculateForeclosure() {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/loans/foreclosure/calculate/${this.foreclosureLoanNumber.trim()}`).subscribe({
      next: (response: any) => {
  ...
  processForeclosure() {
    ...
    this.http.post(`${environment.apiBaseUrl}/api/loans/foreclose/${this.foreclosureLoanNumber.trim()}`, {}).subscribe({
      next: (response: any) => {
  ...
  openEmiDetails(loan: LoanRequest) {
    ...
    const loanId = typeof loan.id === 'number' ? loan.id : Number(loan.id);
    if (isNaN(loanId)) {
      console.error('Invalid loan ID:', loan.id);
      this.loadingEmis = false;
      return;
    }
    this.http.get<any[]>(`${environment.apiBaseUrl}/api/emis/loan/${loanId}`).subscribe({
      next: (emis) => {
        this.emis = emis || [];
        this.loadingEmis = false;
      },
  ...
  viewPersonalLoanForm(loan: LoanRequest) {
    ...
    window.open(`${environment.apiBaseUrl}/api/loans/download-personal-loan-form/${loan.id}`, '_blank');
  }
}
