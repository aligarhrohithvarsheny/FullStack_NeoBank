...
export class AdminGoldLoans implements OnInit {
  ...
  loadGoldLoans() {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/gold-loans`).subscribe({
      next: (loans: any) => {
  ...
  processApproval(loan: GoldLoan, status: string, goldDetails: any) {
    ...
    this.http.put(
      `${environment.apiBaseUrl}/api/gold-loans/${loan.id}/approve?status=${status}&approvedBy=${this.adminName}`,
      requestBody
    ).subscribe({
      next: (response: any) => {
        if (response.success) {
  ...
  loadForeclosureDetails(loan: GoldLoan) {
    this.http.get<any>(`${environment.apiBaseUrl}/api/gold-loans/foreclosure/${loan.loanAccountNumber}`).subscribe({
      next: (response) => {
        if (response.success) {
  ...
  updateLoanInterestRate(loanId: number, newInterestRate: number) {
    this.http.put<any>(`${environment.apiBaseUrl}/api/gold-loans/${loanId}/interest-rate`, null, {
  ...
  processForeclosure() {
    ...
    this.http.post<any>(`${environment.apiBaseUrl}/api/gold-loans/foreclosure/${this.selectedLoan.loanAccountNumber}`, requestBody).subscribe({
      next: (response) => {
        if (response.success) {
  ...
  loadEmiSchedule(loan: GoldLoan) {
    this.http.get<any[]>(`${environment.apiBaseUrl}/api/gold-loans/emi-schedule/${loan.loanAccountNumber}`).subscribe({
      next: (schedule) => {
