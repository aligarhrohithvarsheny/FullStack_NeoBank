...
import { FamilyBankingService, FamilyInvitation, JointTransfer, MinorApplication, JointAccountProfile, FamilyNotification } from '../../../service/family-banking.service';
...
export class FamilyBankingComponent implements OnInit {
  ...
  refresh(): void { this.family.invitations(this.userId).subscribe(v => this.invitations = v); this.family.jointAccounts(this.userId).subscribe(v => this.jointAccounts = v); this.family.pendingTransfers(this.userId).subscribe(v => this.transfers = v); this.family.applications(this.userId).subscribe(v => this.applications = v); this.family.guardianLinks(this.userId).subscribe(v => this.links = v); this.family.notifications(this.userId).subscribe(v => this.notifications = v); }
  ...
  readNotification(notification: FamilyNotification): void { this.family.markNotificationRead(this.userId, notification.id).subscribe(() => this.refresh()); }
