...
export class Transaction implements OnInit {
  ...
  loadTransactionsFromMySQL() {
    ...
    // Load transactions from MySQL database
    this.http.get(`${environment.apiBaseUrl}/api/transactions/account/${this.userAccountNumber}?page=0&size=100`).subscribe({
      next: (response: any) => {
        try {
          ...
          this.error = 'Failed to parse transactions data';
          this.transactions = [];
          this.loading = false;
        }
      },
      error: (err: any) => {
        ...
        if (err.error && typeof err.error === 'string' && err.error.includes('<!DOCTYPE')) {
          this.error = 'Server error: Received HTML instead of JSON. Please check server status.';
        } else if (err.status === 0) {
          this.error = 'Unable to connect to server. Please check your connection.';
        } else if (err.status === 404) {
          this.error = 'Transactions endpoint not found. Please contact support.';
        } else {
  ...
  addTransaction() {
    ...
    this.http.post(`${environment.apiBaseUrl}/api/transactions`, transactionData).subscribe({
      next: (response: any) => {
