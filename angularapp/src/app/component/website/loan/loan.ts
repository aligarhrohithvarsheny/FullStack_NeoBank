...
export class Loan implements OnInit {
  ...
  confirmBulkPayment() {
    ...
    const payNext = (index: number) => {
      ...
      const emiId = typeof emi.id === 'number' ? emi.id : Number(emi.id);
      if (isNaN(emiId)) {
        console.error('Invalid EMI ID:', emi.id);
        return;
      }
      this.http.post(`${environment.apiBaseUrl}/api/emis/${emiId}/pay?accountNumber=${this.userAccountNumber}`, {}).subscribe({
        next: (response: any) => {
          if (response.success) {
            completed++;
          } else {
  ...
  loadUserPanAndCibil() {
    ...
    // First get PAN
    this.http.get(`${environment.apiBaseUrl}/api/loans/user-pan/${this.userAccountNumber}`).subscribe({
      next: (response: any) => {
  ...
  checkCibilByAccount() {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/loans/check-cibil-by-account/${this.userAccountNumber}`).subscribe({
      next: (response: any) => {
        if (response.success) {
  ...
  checkCibilByPan(pan: string) {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/loans/check-cibil/${pan}`).subscribe({
      next: (response: any) => {
        if (response.success) {
          this.cibilInfo.cibilScore = response.cibilScore || 0;
          this.cibilInfo.interestRate = response.interestRate || 0;
...
<body>
  ...
  fetchChildAccountDetails() {
    ...
    const verifyUrl = `${environment.apiBaseUrl}/api/accounts/verify?aadharNumber=${encodeURIComponent(this.childAccountAadhar)}&accountNumber=${encodeURIComponent(this.childAccountNumber)}`;
  ...
  private proceedWithLoanSubmission() {
    ...
    if (this.loanForm.type === 'Personal Loan' && this.personalLoanFormFile) {
      ...
      this.http.post(`${environment.apiBaseUrl}/api/loans/upload-personal-loan-form`, formData).subscribe({
  ...
  downloadForeclosurePDF(loan: LoanRequest) {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/loans/foreclosure/pdf/${loan.loanAccountNumber}`, {
      responseType: 'blob'
    }).subscribe({
      next: (blob: Blob) => {
...
<body>
  ...
  loadEmisForLoan(loanId: number) {
    // Ensure ID is a number
    const loanIdNum = typeof loanId === 'number' ? loanId : Number(loanId);
    if (isNaN(loanIdNum)) {
      console.error('Invalid loan ID:', loanId);
      return;
    }
    this.http.get(`${environment.apiBaseUrl}/api/emis/loan/${loanIdNum}`).subscribe({
      next: (emis: any) => {
  ...
  confirmEmiPayment() {
    ...
    this.http.post(`${environment.apiBaseUrl}/api/emis/${emi.id}/pay?accountNumber=${this.userAccountNumber}`, {}).subscribe({
      next: (response: any) => {
        if (response.success) {
          // Close modal
  ...
  downloadEmiReceipt(emi: EmiPayment) {
    ...
    this.http.get(`${environment.apiBaseUrl}/api/emis/${emiId}/receipt`, {
      responseType: 'blob'
    }).subscribe({
      next: (blob: Blob) => {
        const url = window.URL.createObjectURL(blob);
