...
export class SubsidyClaim implements OnInit {
  ...
  loadEducationLoans() {
    ...
    this.loading = true;
    this.http.get<any>(`${environment.apiBaseUrl}/api/loans/account/${this.userAccountNumber}`).subscribe({
      next: (response) => {
...
<body>
  ...
  submitClaim() {
    ...
    this.subsidyClaimService.createClaim({
      ...
      next: (claim) => {
        // If user has selected a supporting file, upload it and try to auto-detect details
        if (this.selectedFile) {
          this.uploadInProgress = true;
          this.subsidyClaimService.uploadDocument(claim.id!, this.selectedFile).subscribe({
            next: (res) => {
              if (res && res.extractedText) {
                // Try to auto-detect a 12-digit Aadhar number from extracted text
                const match = (res.extractedText || '').match(/\b(\d{12})\b/);
                if (match && match[1]) {
                  this.childAadharNumber = match[1];
                }
              }
              this.alertService.success('Success', 'Subsidy claim submitted and document uploaded successfully! Admin will review your request.');
              this.finishAfterSubmit();
            },
            error: (err) => {
              console.error('Error uploading document:', err);
              this.alertService.warning('Warning', 'Claim submitted but failed to upload document');
              this.finishAfterSubmit();
            }
          });
        } else {
  ...
  onFileSelected(event: any) {
    ...
    if (!validTypes.includes(file.type)) {
      this.alertService.error('Invalid File', 'Please upload a PDF or JPG/JPEG file (max 5MB)');
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      this.alertService.error('File Too Large', 'Maximum allowed file size is 5MB');
      return;
    }
