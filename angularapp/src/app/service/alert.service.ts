...
export class AlertService {
  ...
  /**
   * Backwards-compatible alias for `warning()` used elsewhere as `warn()`.
   */
  warn(title: string, message: string, autoClose: boolean = true, duration: number = 4000): void {
    this.warning(title, message, autoClose, duration);
  }
  ...
  transferConfirm(
    ...
    onConfirm: () => void,
    onCancel?: () => void,
    confirmText: string = 'Confirm Transfer',
    cancelText: string = 'Cancel'
  ): void {
    this.showAlert({
      type: 'confirm',
