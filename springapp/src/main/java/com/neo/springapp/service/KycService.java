        ...
        if (requestOpt.isPresent()) {
            ...
            if (bankChargesService != null && request.getUserAccountNumber() != null && !request.getUserAccountNumber().trim().isEmpty()) {
                try {
                    ...
                    boolean charged = bankChargesService.applyKycCharge(request.getUserAccountNumber(), userName);
                    if (charged) {
                        System.out.println("✅ KYC verification charge applied and credited to branch for: " + request.getUserAccountNumber());
                    }
                } catch (Exception e) {
                    System.err.println("KYC charge could not be applied (branch mapping): " + e.getMessage());
                }
            }
