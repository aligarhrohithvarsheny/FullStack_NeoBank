...
public class UserService {
    ...
    public User approveUser(Long userId, String adminName) {
        ...
        if (userOpt.isPresent()) {
            ...
            // Generate unique account number for approval (mandatory, never duplicate)
            user.setAccountNumber(accountService.generateUniqueAccountNumberForNewAccount());
            
            // Update user status
            user.setStatus("APPROVED");
            // SECURITY: Ensure user must set password after approval
    ...
    /**
     * Close a user account. Admin cannot close if:
     * - Any Loan is still Pending/Approved (not foreclosed/paid/rejected)
     * - Any GoldLoan is still Pending/Approved (not foreclosed/paid/rejected)
     * - Any CreditCard is not Closed
     * - Any cheque draw request is pending/approved (debit in-flight)
     * - Any EducationLoanSubsidyClaim is Pending/Approved
     * - Account balance is not exactly 0 (no auto-transfer on closure)
     *
     * When closing:
     * - User's Account status becomes CLOSED (balance must already be 0)
     * - User status becomes CLOSED
     * - All linked bank cards are auto-blocked/deactivated
     * - User login is blocked (handled elsewhere by CLOSED status)
     */
    public User closeUserAccount(Long userId, String adminName, String adminAccountNumber) {
        Optional<User> userOpt = userRepository.findById(userId);
        if (userOpt.isPresent()) {
            User user = userOpt.get();
            if (user.getAccountNumber() == null || user.getAccountNumber().trim().isEmpty()) {
                throw new RuntimeException("User does not have a bank account number");
            }

            String userAccountNumber = user.getAccountNumber();

            // 1) Block closure if there are pending/active loans (including education child loans)
            List<Loan> loans = loanRepository.findByAccountNumber(userAccountNumber);
            loans.addAll(loanRepository.findByChildAccountNumber(userAccountNumber));
            boolean hasOpenLoan = loans.stream().anyMatch(l ->
                    l.getStatus() != null &&
                            !l.getStatus().equalsIgnoreCase("Foreclosed") &&
                            !l.getStatus().equalsIgnoreCase("Paid") &&
                            !l.getStatus().equalsIgnoreCase("Rejected"));
            if (hasOpenLoan) {
                throw new RuntimeException("Cannot close account: user has pending/active loans");
            }

            // 2) Block closure if gold loan pending/active
            List<GoldLoan> goldLoans = goldLoanRepository.findByAccountNumber(userAccountNumber);
            boolean hasOpenGoldLoan = goldLoans.stream().anyMatch(gl ->
                    gl.getStatus() != null &&
                            !gl.getStatus().equalsIgnoreCase("Foreclosed") &&
                            !gl.getStatus().equalsIgnoreCase("Paid") &&
                            !gl.getStatus().equalsIgnoreCase("Rejected"));
            if (hasOpenGoldLoan) {
                throw new RuntimeException("Cannot close account: user has pending/active gold loans");
            }

            // 2c) Block closure if there are any open education loan subsidy claims
            // We treat Pending/Approved as open; Rejected/Credited are considered closed.
            java.util.List<EducationLoanSubsidyClaim> subsidyClaims =
                    educationLoanSubsidyClaimRepository.findByAccountNumber(userAccountNumber);
            boolean hasOpenSubsidy = subsidyClaims.stream().anyMatch(c ->
                    c.getStatus() != null &&
                            !c.getStatus().equalsIgnoreCase("Rejected") &&
                            !c.getStatus().equalsIgnoreCase("Credited"));
            if (hasOpenSubsidy) {
                throw new RuntimeException("Cannot close account: education loan subsidy claim is pending/approved");
            }

            // 3) Block closure if any credit card not closed
            List<CreditCard> creditCards = creditCardRepository.findByAccountNumber(userAccountNumber);
            boolean hasOpenCreditCard = creditCards.stream().anyMatch(cc ->
                    cc.getStatus() == null || !cc.getStatus().equalsIgnoreCase("Closed"));
            if (hasOpenCreditCard) {
                throw new RuntimeException("Cannot close account: credit card is not closed");
            }

            // 4) Block closure if any cheque draw request is in-flight (debit pending)
            boolean hasPendingChequeDebit = chequeRepository.findByAccountNumber(userAccountNumber).stream().anyMatch(c ->
                    c.getRequestStatus() != null &&
                            ("PENDING".equalsIgnoreCase(c.getRequestStatus()) || "APPROVED".equalsIgnoreCase(c.getRequestStatus())) &&
                            "ACTIVE".equalsIgnoreCase(c.getStatus()));
            if (hasPendingChequeDebit) {
                throw new RuntimeException("Cannot close account: a cheque debit request is pending/approved");
            }

            // Fetch user account record
            Account userAccount = accountService.getAccountByNumber(userAccountNumber);
            if (userAccount == null) {
                throw new RuntimeException("User account not found for account number: " + userAccountNumber);
            }

            if ("CLOSED".equalsIgnoreCase(userAccount.getStatus()) || "CLOSED".equalsIgnoreCase(user.getStatus())) {
                throw new RuntimeException("Account is already closed");
            }

            Double balance = userAccount.getBalance() != null ? userAccount.getBalance() : 0.0;
            if (balance < 0) {
                throw new RuntimeException("Cannot close account: user balance is negative");
            }

            // Require zero balance: no automatic transfers on closure.
            if (balance > 0) {
                throw new RuntimeException("Cannot close account: balance must be 0 before closure. Current balance: " + balance);
            }

            // Auto-block all regular bank cards (debit/ATM) linked to this account
            java.util.List<Card> cards = cardRepository.findByAccountNumber(userAccountNumber);
            for (Card card : cards) {
            ...
            // Optional: record who closed; User model currently doesn't have closedBy field
            User savedUser = userRepository.save(user);

            // Send account closure email notification (best-effort)
            try {
                if (savedUser.getEmail() != null && !savedUser.getEmail().trim().isEmpty()) {
                    String username = savedUser.getUsername() != null ? savedUser.getUsername() :
                            (savedUser.getAccount() != null ? savedUser.getAccount().getName() : "Customer");
                    // Reuse existing login-notification style sending with a custom body
                    String closureTime = java.time.LocalDateTime.now().toString();
                    String body = "Dear " + username + ",\n\n" +
                            "Your NeoBank account (" + userAccountNumber + ") has been successfully closed on " + closureTime + ".\n\n" +
                            "Key points:\n" +
                            "- Your internet banking login has been disabled.\n" +
                            "- Linked debit cards have been blocked.\n" +
                            "- No further transactions can be performed on this account.\n\n" +
                            "If you did not request this closure, please contact our support team immediately.\n\n" +
                            "Thank you for banking with NeoBank.\n\n" +
                            "Best regards,\n" +
                            "NeoBank Customer Service Team";

                    // Use a lightweight send via OTP email infrastructure (development-friendly)
                    emailService.sendOtpEmailWithReason(savedUser.getEmail(), "ACCOUNT_CLOSED", "Account Closure Notification");
                    System.out.println("Account closure email body (for logging only):\n" + body);
                }
            } catch (Exception e) {
                System.out.println("⚠️ Failed to send account closure email: " + e.getMessage());
            }
