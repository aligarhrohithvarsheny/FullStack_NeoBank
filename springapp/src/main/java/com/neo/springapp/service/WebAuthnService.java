...
public class WebAuthnService {
    ...
    public Map<String, Object> authenticate(String adminEmail, Map<String, Object> authenticationData) {
        // Check if admin exists and is not locked
        Admin admin = adminRepository.findByEmail(adminEmail);
        if (admin == null) {
            throw new RuntimeException("Admin not found");
        }
        if (admin.getAccountLocked() != null && admin.getAccountLocked()) {
            ...
            lockedResponse.put("message", "Account is locked due to 3 failed login attempts. Please contact manager to unlock.");
        ...
        try {
            ...
            // Track failed fingerprint attempt
            ...
            admin.setLastFailedLoginTime(java.time.LocalDateTime.now());
            if (admin.getFailedLoginAttempts() >= 3) {
                ...
                lockedResponse.put("success", false);
                lockedResponse.put("accountLocked", true);
                lockedResponse.put("message", "Account locked due to 3 failed attempts. Please contact manager to unlock.");
                return lockedResponse;
            }
            adminRepository.save(admin);
            throw e;
        ...
        // Reset failed login attempts on successful fingerprint auth
