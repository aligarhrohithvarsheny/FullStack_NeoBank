...
public class PositivePayService {
    ...
    public List<Map<String, Object>> eligibleCheques(String accountNumber) {
        ...
        if (owner.type.equals("SAVINGS")) {
            ...
            if (salary != null) for (ChequeRequest cheque : salaryChequeRepository.findBySalaryAccountIdOrderByCreatedAtDesc(salary.getId(), org.springframework.data.domain.PageRequest.of(0, 200)).getContent()) if ("APPROVED".equalsIgnoreCase(cheque.getStatus())) result.add(chequeMap(cheque.getId(), "SALARY_CHEQUE_REQUESTS", cheque.getChequeNumber(), accountNumber, owner.type, owner.name, cheque.getAmount() == null ? null : cheque.getAmount().doubleValue(), cheque.getStatus(), cheque.getSerialNumber(), cheque.getAmount() != null && cheque.getAmount().compareTo(minimumAmount) >= 0));
        } else {
            ...
            for (BusinessChequeRequest cheque : businessChequeRepository.findByCurrentAccountIdOrderByCreatedAtDesc(current.getId(), org.springframework.data.domain.PageRequest.of(0, 200)).getContent()) if ("APPROVED".equalsIgnoreCase(cheque.getStatus())) result.add(chequeMap(cheque.getId(), "BUSINESS_CHEQUE_REQUESTS", cheque.getChequeNumber(), accountNumber, owner.type, owner.name, cheque.getAmount() == null ? null : cheque.getAmount().doubleValue(), cheque.getStatus(), cheque.getSerialNumber(), cheque.getAmount() != null && cheque.getAmount().compareTo(minimumAmount) >= 0));
        }
