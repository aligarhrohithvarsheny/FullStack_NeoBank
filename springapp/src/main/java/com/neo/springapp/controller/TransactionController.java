...
public class TransactionController {
    ...
    // ATM Withdrawal — debit balance and record transaction
    @PostMapping("/withdraw")
    public ResponseEntity<Map<String, Object>> atmWithdraw(@RequestBody Map<String, Object> request) {
        Map<String, Object> response = new HashMap<>();
        try {
            String accountNumber = (String) request.get("accountNumber");
            Double amount = Double.valueOf(request.get("amount").toString());
            String description = (String) request.getOrDefault("description", "ATM Cash Withdrawal");
            ...
            if (newBalance == null) {
                response.put("success", false);
                response.put("message", "Insufficient balance or account not found");
                return ResponseEntity.badRequest().body(response);
            }
            ...
            // Try to set userName from user service
            Optional<User> userOpt = userService.getUserByAccountNumber(accountNumber);
            userOpt.ifPresent(u -> txn.setUserName(u.getName()));
    ...
    // Get recent transactions for an account (used by ATM mini-statement)
    @GetMapping("/recent/{accountNumber}")
    public ResponseEntity<List<Transaction>> getRecentTransactions(@PathVariable String accountNumber) {
