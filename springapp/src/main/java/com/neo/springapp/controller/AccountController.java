...
public class AccountController {
    ...
    public ResponseEntity<Map<String, Object>> verifyAccountUniversal(@PathVariable String accountNumber) {
        Map<String, Object> result = accountService.verifyAccountByNumber(accountNumber);
    ...
    public ResponseEntity<Map<String, Object>> debitBalance(
        ...
        try {
            ...
            // Handle closed account exception
            if (e.getMessage() != null && e.getMessage().contains("closed account")) {
                response.put("error", "Cannot perform withdrawal on a closed account");
    ...
    public ResponseEntity<Map<String, Object>> creditBalance(
        ...
        try {
            ...
            // Handle closed account exception
            if (e.getMessage() != null && e.getMessage().contains("closed account")) {
                System.out.println("❌ Cannot deposit to closed account: " + accountNumber);
                response.put("error", "Cannot perform deposit on a closed account");
                response.put("message", e.getMessage());
                return ResponseEntity.badRequest().body(response);
            }
            response.put("error", e.getMessage());
    ...
    /**
     * Generate a customerId for an existing account (admin action).
     * Can only be called once per account; if a customerId already exists, it returns a clear error.
     */
    @PostMapping("/generate-customer-id/{accountNumber}")
    public ResponseEntity<Map<String, Object>> generateCustomerIdForExistingAccount(@PathVariable String accountNumber) {
        Map<String, Object> response = new HashMap<>();
        try {
            Account account = accountService.getAccountByNumber(accountNumber);
            if (account == null) {
                response.put("success", false);
                response.put("message", "Account not found for accountNumber: " + accountNumber);
                return ResponseEntity.status(404).body(response);
            }

            if (account.getCustomerId() != null && !account.getCustomerId().trim().isEmpty()) {
                response.put("success", false);
                response.put("message", "Customer ID is already assigned for this account.");
                response.put("customerId", account.getCustomerId());
                return ResponseEntity.badRequest().body(response);
            }

            // Generate and assign a fresh customerId
            String customerId = accountService.generateCustomerIdForAccount(account);
            account.setCustomerId(customerId);
            account.setLastUpdated(LocalDateTime.now());
            Account saved = accountService.saveAccount(account);

            response.put("success", true);
            response.put("message", "Customer ID generated successfully");
            response.put("customerId", saved.getCustomerId());
            response.put("accountNumber", saved.getAccountNumber());
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", "Failed to generate Customer ID: " + e.getMessage());
            return ResponseEntity.internalServerError().body(response);
        }
    }
}
