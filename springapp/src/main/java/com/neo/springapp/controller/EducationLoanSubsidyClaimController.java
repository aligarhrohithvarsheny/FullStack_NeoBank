...
public class EducationLoanSubsidyClaimController {
    ...
    /**
     * Upload a supporting document for a claim (PDF/JPG/JPEG, max 5MB).
     */
    @PostMapping("/{id}/upload-document")
    public ResponseEntity<?> uploadClaimDocument(@PathVariable Long id, @RequestParam("file") MultipartFile file) {
        try {
            Map<String, Object> result = subsidyClaimService.uploadClaimDocument(id, file);
            return ResponseEntity.ok(result);
        } catch (IOException e) {
            Map<String, Object> error = new HashMap<>();
            error.put("success", false);
